...
flowchart TD
    ...
    %% Style
    style GD fill:#fdf4ff,stroke:#d946ef,stroke-width:2px,color:#000
    style Verify fill:#fef9c3,stroke:#ca8a04,stroke-width:2px,color:#000
    style DWH fill:#dcfce7,stroke:#22c55e,stroke-width:2px,color:#000
```
...
| Tên dịch vụ | Nhiệm vụ trong hệ thống | Cổng local | Database | User mặc định | Volume dữ liệu |
| :--- | :--- | :---: | :--- | :--- | :--- |
...
Khởi tạo cấu trúc bảng nghiệp vụ nguồn và nạp dữ liệu. Script tự apply `sql/source_mysql_hr/01_driver_tables.sql`, nên thành viên mới không cần chạy lệnh MySQL DDL thủ công:
```powershell
...
```
> [!NOTE]
> `00_create_schema.sql` chỉ dùng cho trường hợp nâng cao khi tự dựng MySQL ngoài Docker Compose. Với `docker-compose.yml` của repo, database `green_taxi_hr` đã được container tạo từ biến `MYSQL_HR_DATABASE`.
...
# Áp dụng cấu trúc bảng cho staging, audit, dq, NDS và DDS
python scripts/apply_warehouse_ddl.py --mode docker
```
*Chi tiết DDL executable và mô hình vật lý xem tại [Warehouse Physical Model Specification](14-warehouse-ddl.md).*
...
```powershell
# Nên smoke test từng nguồn trước khi tiến hành full load
...
Khi các service local đã khởi động, bạn có thể mở Streamlit để kiểm tra health và dữ liệu nguồn. Các khu vực row count, batch history, DQ và reconciliation chỉ có dữ liệu đầy đủ sau khi schema tương ứng đã được tạo và pipeline đã chạy:
...
```
> [!NOTE]
> Dry-run trả run-level status `DRY_RUN`, exit code `0` và không đánh dấu DDS sẵn sàng cho BI.
...
*   **Mục tiêu:** Chạy trích xuất toàn bộ dữ liệu thực tế, seed nguồn, nạp Staging/NDS/DDS và đối soát chất lượng dữ liệu trên quy mô đầy đủ. Loader NDS/DDS đã được triển khai; full smoke test trên môi trường sạch và báo cáo BI vẫn là công việc cần hoàn tất.
