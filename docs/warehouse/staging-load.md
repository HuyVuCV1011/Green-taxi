...
Tài liệu này mô tả architecture, configuration, mapping logic, DQ controls,
idempotency và reconciliation của quá trình load dữ liệu từ source systems vào
...
## 5. Audit Logging And Reconciliation
...
- CLI hiển thị bảng tổng hợp chi tiết reconciliation sau khi chạy.

## Trạng thái triển khai

Staging loader, source adapters và audit row count đã được triển khai. Có thể
chạy riêng bằng `scripts/load_staging.py` hoặc qua `PipelineRunner`. Full-release
source-to-staging, NDS/DDS reconciliation và idempotency đã có evidence tại
[../evidence/full-release-reconciliation.md](../evidence/full-release-reconciliation.md)
và trạng thái tích hợp hiện hành tại
[../evidence/integration-review.md](../evidence/integration-review.md).
