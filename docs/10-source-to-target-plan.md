...
5. **Source-system lineage**:
   - Dùng mã văn bản ổn định từ staging (`HR_MYSQL`, `FLEET_MONGODB`,
     `DISPATCH_POSTGRES`, `TLC_FILE`, `LOOKUP_FILE`).
   - Mã phải tồn tại trong `nds.ref_source_system`; không dùng magic integer.
6. **Trip business key**:
   - `staging.trip_key` được chuyển nguyên vẹn sang `nds.nds_trip.trip_nk`
     dưới kiểu `TEXT`.
   - Không cắt, băm lại hoặc thay đổi biểu diễn khóa trong các tầng sau.
...
| `source_system` | `source_system_code`| `VARCHAR(50)` | Giữ mã `HR_MYSQL`, validate FK với `nds.ref_source_system`. |
...
| *Tự sinh* | `is_inferred` | `BOOLEAN` | Mặc định `false`. Set `true` nếu là dòng skeleton do late-arriving. |
| `source_system` | `source_system_code`| `VARCHAR(50)` | Giữ mã `FLEET_MONGODB`, validate FK với `nds.ref_source_system`. |
...
* **Tài liệu hóa luồng di chuyển**:
  - `Staging.shifts` $\rightarrow$ `NDS.nds_shift` $\rightarrow$ `DDS.fact_driver_shift`
...
Không tạo `dim_shift`. Với grain hiện tại, mỗi shift chỉ tương ứng một dòng
`fact_driver_shift`; tạo thêm dimension sẽ lặp lại cùng thuộc tính và thêm join
1:1. `shift_id` được giữ trực tiếp trong fact như một degenerate dimension.
...
|---|---|---|---|
| `nds_shift.shift_nk` | `shift_id` | `VARCHAR(50)` | Degenerate dimension và business key duy nhất của fact. |
...
| `nds_shift.shift_status` | `shift_status` | `VARCHAR(50)` | Trạng thái ca tại thời điểm load. |
| `nds_shift.shift_start` | `shift_start` | `TIMESTAMP` | Timestamp nghiệp vụ New York. |
| `nds_shift.shift_end` | `shift_end` | `TIMESTAMP` | Timestamp nghiệp vụ New York. |
| *DQ Gate 2* | `is_anomaly` | `BOOLEAN` | `true` nếu ca vi phạm overlap driver/vehicle. |
...
|---|---|---|---|
| `trip_key` | `trip_nk` | `TEXT` | Giữ nguyên full business key từ staging, không truncate hoặc băm lại. |
...
| `trip_distance` | `trip_distance` | `DECIMAL(12,4)` | Khoảng cách (mile), đủ rộng để bảo toàn outlier nguồn. |
...
|---|---|---|---|
| `nds_trip.trip_nk` | `trip_id` | `TEXT` | Natural key đầy đủ của trip làm **Degenerate Dimension**. |
| `nds_shift.shift_nk` | `shift_id` | `VARCHAR(50)` | Degenerate dimension; lookup qua `nds_trip_assignment.shift_sk`. |
...
| `nds_trip.trip_distance` | `trip_distance` | `DECIMAL(12,4)`| Lưu trực tiếp measure, không thu hẹp precision từ NDS. |
