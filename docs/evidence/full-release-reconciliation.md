...
Status: `CURRENT FULL-RELEASE VALIDATION EVIDENCE`
...
Duong dan tren la moi truong validation cu the. Onboarding mac dinh copy hai thu
muc `tlc` va `synthetic` vao `data/raw/`; cac script van chap nhan layout khac
neu thiet lap `DATA_ROOT` phu hop.
...
Bang nay phan anh database reconciliation `green_taxi_warehouse_reconciliation_v1`
tai thoi diem chay idempotency. DQ fixture duoc kiem tra rieng o muc rule; clean
snapshot hien hanh de bao cao tich hop nam trong
