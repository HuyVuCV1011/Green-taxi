...
-- Create/login/password must be supplied by an administrator outside this file.
-- Example only: CREATE ROLE superset_ro WITH LOGIN;
...
GRANT USAGE ON SCHEMA dds, analytics TO superset_ro;
...
ALTER DEFAULT PRIVILEGES FOR ROLE green_taxi_warehouse_app IN SCHEMA dds
...
-- Không cấp quyền trực tiếp trên staging, nds, audit hoặc dq.
