...
|---|---|---|
| Quyết định mối quan hệ active/inactive cho các chiều có nhiều liên kết | Xác định mối quan hệ nào giữa `fact_driver_trip` và `dim_date` (pickup hay dropoff) hoặc `dim_location` (pickup hay dropoff) là **Active** mặc định trong Power BI semantic model | `TBD_AFTER_RECONCILIATION` |
...
| Cách tính `utilization_rate` cấp độ tổng hợp (Aggregation) | Xác định measure trong Power BI là `SUM(occupied_minutes) / SUM(shift_duration_minutes)` để tránh lỗi tính trung bình của các tỷ lệ | `TBD_AFTER_RECONCILIATION` |
