...
Các analytics views expose thêm nhãn `*_weekday_label` dạng `1 - Monday` để
dashboard giữ đúng thứ tự ngày trong tuần trên chart category. Nhãn này chỉ
phục vụ hiển thị; công thức metric vẫn dùng `*_day_of_week`, `*_day_name` và
timestamp/date role theo dataset ownership ở trên.
