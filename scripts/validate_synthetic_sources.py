...
def validate(
    ...
    with (synthetic_root / "driver_hr" / "drivers.csv").open(
        ...
        for row in csv.DictReader(handle):
            ...
            driver_vendor[row["driver_id"]] = int(row["vendor_id"])
    ...
    with (synthetic_root / "fleet" / "vehicles.jsonl").open(encoding="utf-8") as handle:
        for line in handle:
            ...
            vehicle_vendor[record["vehicle_id"]] = int(record["vendor_id"])
    ...
    report = {
        ...
        "notes": [
            ...
            "Driver and vehicle temporal overlap were validated across all months.",
