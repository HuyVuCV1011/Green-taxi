...
class NDSLoader:
    ...
    def write_quarantine(self, conn: Any, table_name: str, error_rule_code: str, columns: list[str], row_data: tuple) -> None:
        """Persist an ERROR record without coupling ETL to source-specific DDL."""
        payload = dict(zip(columns, row_data))
        source_system = payload.get("source_system")
        source_entity = payload.get("source_entity") or table_name
        source_record_id = payload.get("source_record_id") or payload.get("trip_key")
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO dq.quarantine_record (
                    batch_id, release_id, source_system_code, source_entity,
    ...
    def load_vendor(self) -> tuple[int, int, int]:
        ...
        dwh.commit()
        status = "SUCCEEDED" if loaded + quarantined == read_count else "FAILED"
    ...
    def load_trips(self, limit_rows: int | None = None) -> tuple[int, int, int]:
        ...
        while True:
            ...
            with dwh.cursor() as cur:
                cur.execute("""
                    SELECT t.batch_id, t.release_id, t.source_system, t.source_entity,
                           t.source_locator, t.source_file, t.source_row_number,
                           t.source_extract_at, t.load_timestamp, t.source_checksum,
                           t.extraction_watermark, t.row_hash, t.vendor_id,
                           t.lpep_pickup_datetime, t.lpep_dropoff_datetime,
                           t.store_and_fwd_flag, t.ratecode_id, t.pu_location_id,
                           t.do_location_id, t.passenger_count, t.trip_distance,
                           t.fare_amount, t.extra, t.mta_tax, t.tip_amount,
                           t.tolls_amount, t.ehail_fee,
                           t.improvement_surcharge, t.total_amount, t.payment_type,
                           t.trip_type, t.congestion_surcharge, ta.trip_key,
                           t.staging_row_id
                    FROM staging.stg_tlc_green_trips t
                    JOIN staging.stg_dispatch_trip_assignments ta
                      ON ta.release_id = t.release_id
                     AND ta.source_file = t.source_file
                     AND ta.source_row_number = t.source_row_number
                    WHERE t.release_id = %s
                      AND t.staging_row_id > %s
                    ORDER BY t.staging_row_id
                    LIMIT %s;
                """, (self.release_id, last_staging_row_id, current_batch_size))
                rows = cur.fetchall()
            ...
            for row in rows:
                ...
                chunk.append((
                    ...
                    trip_type, source_file, source_row_number, False, source_system
                ))
